...
public class IngredientController {
    ...
    @GetMapping("/name")
    public ResponseEntity<Set<Ingredient>> getIngredientsByName(@PathVariable("name") String nameIngredient){
        Set<Ingredient> ingredients = ingredientService.findIngredientsByNameLike(nameIngredient);
        return new ResponseEntity<>(ingredients, HttpStatus.OK);
    }

    @GetMapping("/category")
    public ResponseEntity<Set<Ingredient>> getIngredientsByCategory(@PathVariable("category") CategoryIngredient categoryIngredient){
        //Ver de validar la categoria de alguna manera
    ...
    public ResponseEntity<Ingredient> modifyIngredient(@PathVariable("id") Long idIngredient, @RequestBody Ingredient newIngredient){
        Ingredient ingredient = ingredientService.modifyIngrediet(idIngredient, newIngredient);
        return new ResponseEntity<>(ingredient, HttpStatus.OK);
    }
    ...
    handleException(IngredientNotFoundException pnfe) {
